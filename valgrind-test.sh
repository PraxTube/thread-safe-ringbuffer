...
for test_executable in "${test_executables[@]}"; do
  if [ ! -f $test_executable ]; then
    ...
    valgrind --error-exitcode=1 $test_executable > /dev/null
    if [ $? -eq 0 ]; then
...
for test_executable in "${test_executables_by_file[@]}"; do
  ...
  if [ ! -f $test_executable ]; then
    ...
    echo -e "\n===> \033[0;34mRUNNING\033[0m $test_executable <===\n"
    timeout 2s valgrind --error-exitcode=1 $test_executable ./test/testfile.txt "$dest_file" > /dev/null
    if [ $? -eq 0 ]; then
			echo "echo -e \"\033[0;32mPASSED\033[0m --- $test_executable\"" >> $summary_file
      passed=$((passed+1))
    else
...
for test_executable in "${test_executables_threaded[@]}"; do
  if [ ! -f $test_executable ]; then
    ...
    echo -e "\n===> \033[0;34mRUNNING\033[0m $test_executable <===\n"
    timeout 11s valgrind --error-exitcode=1 $test_executable > /dev/null
    if [ $? -eq 0 ]; then
			echo "echo -e \"\033[0;32mPASSED\033[0m --- $test_executable\"" >> $summary_file
      passed=$((passed+1))
    else
